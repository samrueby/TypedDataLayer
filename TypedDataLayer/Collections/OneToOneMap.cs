...
namespace TypedDataLayer.Collections {
	...
	public class OneToOneMap<T1, T2> {
		...
		/// </summary>
		public T1 GetLeftFromRight( T2 right ) => rightToLeft[ right ];

		/// <summary>
		/// Given the left value of a pair, returns the corresponding right value.
		/// </summary>
		public T2 GetRightFromLeft( T1 left ) => leftToRight[ left ];

		/// <summary>
		/// Returns a collection of all of the left values.
		/// </summary>
		public ICollection<T1> GetAllLeftValues() => leftValues.AsReadOnly();

		/// <summary>
		/// Returns a collection of all of the right values.
		/// </summary>
		public ICollection<T2> GetAllRightValues() => rightValues.AsReadOnly();

		/// <summary>
		/// Returns a list of (left, right) key value pairs.
		/// </summary>
		public ICollection<KeyValuePair<T1, T2>> GetAllPairs() => pairs.AsReadOnly();
	}
}