	...
	public class Cache<KeyType, ValType> {
		...
		public Cache( bool isThreadSafe, IEqualityComparer<KeyType> comparer = null ) =>
			dictionary = isThreadSafe
				             ? comparer != null ? (IDictionary<KeyType, ValType>)new ConcurrentDictionary<KeyType, ValType>( comparer ) : new ConcurrentDictionary<KeyType, ValType>()
		...
		/// <summary>
		/// Returns the value associated with the given key. If there is no value cached for the given key yet, the value is
		/// created and added to the cache, then
		/// returned.
		/// </summary>
		...
		/// <summary>
		/// Attempts to remove the key/value pair with the specified key. Returns true if the key was found and the key/value pair
		/// was removed.
		/// </summary>
